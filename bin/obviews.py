...
class SourceManager:
	...
	def __init__(self, execdirs = [], paths = ["."]):
		self.execdirs = execdirs
		self.paths = paths
		self.map = {}
		self.corepath_map = {}
		self.sources = []
	...
	def find_actual_path(self, path):
		...
		if os.path.isabs(path):
			...
			return self.identify_valid_path(path, self.paths+self.execdirs)
...
class CFG:
	...
	def gen(self, dec, out):
		...
		dec.start_cfg(self)
		out.write("digraph %s {\n" % self.id)
		out.write('node [ fontname = "Archivo" , margin="0.2,0.055", shape="box",style="rounded, filled", penwidth="1" , color="#3B90F3" , fillcolor="#FFFFFF" ]\n')
...
class Task:
	...
	def __init__(self, execdirs, name, path, source_path = None):
		self.name = name
		self.path = path
		self.entry = None
		self.cfgs = []
		...
		self.sum = Data()
		self.stats = []
		self.sman = SourceManager(execdirs, source_path.split(',') if source_path else ['.'])
		self.read()
		self.defs = None
		self.views = []
		self.sview = None
...
INDEX_MAP = {
	...
	"application":	lambda: os.path.basename(TASK.path),
...
def main():
	...
	parser.add_argument('--executable', default="", type=str,
		help="Select the executable to get statistics from, if multiple executables separate them with a ',' in one string.")
	parser.add_argument('--task', type=str, nargs="?", default="main",
	...
	parser.add_argument("--work-dir", type=str, default=None,
		help="Specify in which directory the files generated by otawa are")
	...
	exe_dirs = [ os.path.dirname(os.path.splitext(executable)[0]) for executable in args.executable.split(",") ]
	exe_names = [ os.path.basename(os.path.splitext(executable)[0]) for executable in args.executable.split(",") ]
	task_name = args.task
	if args.work_dir:
		task_dir = os.path.join(args.work_dir, task_name)
	else:
		if len(exe_dirs) > 1:
			fatal("Don't know how to handle multiple executables without a uniq work-dir") 
		task_dir = os.path.join(exe_dirs[0], exe_names[0] + "-otawa", task_name)
	if not os.path.exists(task_dir):
		fatal("No statistics for %s task %s. (%s)\nDid you forget --stats option in owcet?" % (args.executable, task_name, task_dir))
	TASK = Task(exe_dirs, task_name, task_dir, args.source)
